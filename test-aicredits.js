...
const client = new sdk_1.default({ apiKey, baseURL });
const MODELS = {
    free: {
...
async function testValidation(tier, testNum, goal, code, maxTokens = 300) {
    ...
    try {
        ...
        const costUSD = calculateCostUSD(response.usage.input_tokens, response.usage.output_tokens, model.input_price, model.output_price);
...
async function main() {
    ...
    console.log('═'.repeat(52));
    const test1 = await testValidation('free', 1, 'Add email validation to login form', `const validateEmail = (email) => {
  return /^[^@]+@[^@]+\\.[^@]+$/.test(email);
};`);
    const test2 = await testValidation('pro', 2, 'Parse JSON with error handling', `const parseJSON = (jsonStr) => {
  try {
    ...
    const test3 = await testValidation('max', 3, 'Implement user authentication with JWT', `const jwt = require("jsonwebtoken");
...
main();
//# sourceMappingURL=test-aicredits.js.map