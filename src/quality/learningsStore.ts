...
const DEFAULT_HEADER = [
  '# Tyne Learnings',
  '',
  'Team-maintained review rules, reviewed in pull requests like any other',
  'config. Two sections, both optional.',
  '',
  '## Suppress — findings Tyne should stop reporting',
  '',
  'One bullet per finding title, optionally " — " a reason, optionally a',
  'trailing path glob in parentheses to limit where it applies. Matching,',
  'strongest first: exact title, exact title within the glob, exact ruleId,',
  'then a conservative concept match for longer titles so a learning survives',
  'the model rephrasing a finding. Security, compliance, breaking-change and',
  'scope findings are never suppressed by the concept match — those require an',
  'exact title. Everything suppressed still appears in the review under',
  '"Checked but not shown", with the learning that hid it.',
  '',
  '## Require — conventions Tyne should enforce',
  '',
  'One bullet per convention, in a sentence, optionally with a trailing path',
  'glob. These produce findings rather than hiding them. They are checked by',
  'the model rather than a detector, so they are reported as team preferences',
  'and never as proven defects.',
  '',
  '(Every "-" line under a section heading is a live entry — do not add',
  'example or placeholder bullets, only real ones.)',
].join('\n');
