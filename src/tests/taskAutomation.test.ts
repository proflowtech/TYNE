...
test('getAutomationSettings: returns defaults when no settings stored', () => {
  ...
  assert.equal(s.autoPostFeedbackAfterClose, true);
...
test('tie-the-knot ship close: manual closes, disabled does not', () => {
  const shipClose = (trigger: string) => trigger !== 'disabled';
  assert.equal(shipClose('manual'), true);
  assert.equal(shipClose('on_push'), true);
  assert.equal(shipClose('manual_and_on_push'), true);
  assert.equal(shipClose('disabled'), false);
  const fs = require('node:fs');
  const path = require('node:path');
...
test('formatFeedbackBody: commit shows the URL when provided', () => {
  ...
  assert.ok(body.includes('Commit: https://github.com/org/repo/commit/a1b2c3d4'));
...
test('formatFeedbackBody: Max tier uses Max report sections', () => {
  ...
  assert.ok(body.includes('Validation report'));
  assert.ok(body.includes('Validation stages') || body.includes('AXIOM review'));
  assert.ok(body.includes('**Commit:**') || body.includes('Commit:'));
  assert.ok(body.split(/\s+/).length <= 220);
